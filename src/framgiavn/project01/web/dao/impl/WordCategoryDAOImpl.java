...
public class WordCategoryDAOImpl extends HibernateDaoSupport implements WordCategoryDAO {
	...
	public List<WordCategory> getListWordByCategory(String category) throws Exception {
		...
		List<WordCategory> listWordCategory = new ArrayList<WordCategory>();
		Query query;
		String hql = "select word.id as wordCategoryId, word.content as contentWord, category.name as category, wordAnswer.content as contentResult "
				+ "From Word word, WordAnswer wordAnswer, Category category Where wordAnswer.correct = :correctAnswer and wordAnswer.wordId "
				+ "in ( select word.id From Word Where word.categoryId in ( select category.id From Category ))";
		if (!category.isEmpty() && category != null) {
			hql += " and category.name = :category ";
			query = getSession().createQuery(hql).setParameter("correctAnswer", CORRECT_ANSWER)
					.setParameter("category", category).setParameter("category", category);
